...
clean:
	...
	rm -f src/test/resources/model_core.out
...
install: clean uninstall
	...
	chmod +x $(INSTALL_PATH)/$(PROJECT_NAME)/model_core.out
...
test_prepare:
	make -C src/main/cpp main
	cp src/main/cpp/model_core.out src/test/resources/model_core.out
	chmod +x src/test/resources/model_core.out

test: test_prepare
	./mvnw test

report: test
	open -a 'google chrome' target/site/jacoco/edu.school.calc/Model.html